...
function makeMockDriver(): Driver & {
  publishCalls: OrthoEvent[];
  startCalls: Map<string, EventHandler[]>[];
  stopCallCount: number;
} {
  const publishCalls: OrthoEvent[] = [];
  const startCalls: Map<string, EventHandler[]>[] = [];
  let stopCallCount = 0;
...
describe('EventBusImpl', () => {
  ...
  it('subscribe after start throws', async () => {
    await bus.start();
    expect(() => bus.subscribe('lead.created', vi.fn())).toThrow(
      'subscribe() called after start()',
    );
  });
  ...
  it('multiple handlers for same event type are both present in the map', async () => {
    const h1: EventHandler = vi.fn();
    const h2: EventHandler = vi.fn();
