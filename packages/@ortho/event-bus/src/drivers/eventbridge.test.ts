...
import type { OrthoEvent } from '../types.js';
...
vi.mock('@aws-sdk/client-sqs', () => ({
  ...
  SendMessageCommand: vi.fn((input: unknown) => ({ _type: 'SendMessage', input })),
  ReceiveMessageCommand: vi.fn((input: unknown) => ({ _type: 'ReceiveMessage', input })),
  DeleteMessageCommand: vi.fn((input: unknown) => ({ _type: 'DeleteMessage', input })),
}));
...
function makeDriver(): InstanceType<typeof EventBridgeDriver> {
  process.env['EVENT_BRIDGE_BUS_NAME'] = 'ortho-bus';
  process.env['SQS_QUEUE_URL'] = QUEUE_URL;
  return new EventBridgeDriver();
}

function sqsMessage(event: OrthoEvent, receiptHandle = 'rh-1') {
...
describe('EventBridgeDriver', () => {
  ...
  it('publish sends correct MessageBody and MessageGroupId (entity_id)', async () => {
    ...
    const [cmd] = mockSend.mock.calls[0] as [{ _type: string; input: Record<string, unknown> }];
  ...
  it('publish falls back to event_type for MessageGroupId when entity_id is absent', async () => {
    mockSend.mockResolvedValueOnce({});
    const driver = makeDriver();
    const event: OrthoEvent = { event_type: 'system.ping', payload: {} };
  ...
  it('consume with matching handler calls handler and deletes message', async () => {
    ...
    const deleteCall = mockSend.mock.calls.find(
      (args) => (args[0] as { _type: string })._type === 'DeleteMessage',
    );
  ...
  it('consume with no registered handler deletes message without calling any handler', async () => {
    const subscriptions = new Map<string, never[]>(); // empty
