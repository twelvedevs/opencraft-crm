...
import type { EventHandler, OrthoEvent } from '../types.js';
...
function makeMessage(
  id: string,
  event: Partial<OrthoEvent> = {},
): [string, Array<[string, Array<[string, string[]]>]>] {
  const full: OrthoEvent = {
  ...
  if (full.entity_type) fields.push('entity_type', full.entity_type);
  if (full.entity_id) fields.push('entity_id', full.entity_id);
  return ['stream:test.event', [[id, fields]]] as unknown as [string, Array<[string, Array<[string, string[]]>]>];
}

function xreadResult(id: string, event?: Partial<OrthoEvent>) {
...
describe('RedisStreamsDriver', () => {
  ...
  it('publish calls XADD with correct stream name and MAXLEN', async () => {
    ...
    const event: OrthoEvent = { event_type: 'lead.created', payload: { id: 1 }, entity_id: 'e1' };
    await driver.publish(event);

    expect(mockRedis.xadd).toHaveBeenCalledOnce();
    const args = mockRedis.xadd.mock.calls[0] as unknown[];
    ...
    const fields = args.slice(5) as string[];
  ...
  it('start creates consumer group (XGROUP CREATE with MKSTREAM)', async () => {
    const driver = new RedisStreamsDriver(OPTIONS);
    const subs = new Map<string, EventHandler[]>([['test.event', [vi.fn()]]]);
    await driver.start(subs);
    await driver.stop();

    expect(mockRedis.xgroup).toHaveBeenCalledWith(
      'CREATE', 'stream:test.event', OPTIONS.consumerGroup, '$', 'MKSTREAM',
    );
  });
  ...
  it('message received calls handlers in series and XACKs on success', async () => {
    ...
    const driver = new RedisStreamsDriver(OPTIONS);
    const subs = new Map<string, EventHandler[]>([['test.event', [handler]]]);
    await driver.start(subs);
  ...
  it('calls multiple handlers in series for same event type', async () => {
    ...
    const driver = new RedisStreamsDriver(OPTIONS);
    const subs = new Map<string, EventHandler[]>([['test.event', [h1, h2]]]);
    await driver.start(subs);
    await new Promise<void>((r) => setTimeout(r, 20));
    await driver.stop();
  ...
  it('handler throw results in no XACK when delivery count < 3', async () => {
    ...
    mockRedis.xpending.mockImplementation((...args: unknown[]) => {
      if (args.length === 5) return Promise.resolve([['1-0', 'consumer', 100, 1]]);
  ...
  it('after 3 delivery attempts moves message to DLQ stream and XACKs', async () => {
    ...
    mockRedis.xpending.mockImplementation((...args: unknown[]) => {
      if (args.length === 5) return Promise.resolve([['1-0', 'consumer', 100, 3]]);
      return Promise.resolve([0, null, null, null]);
    });
    mockRedis.xreadgroup
    ...
    // Should add to DLQ
    const dlqCall = mockRedis.xadd.mock.calls.find(
      (c) => (c as unknown[])[0] === 'stream:dlq',
    );
  ...
  it('pending count > 1000 logs consumer-lag warning', async () => {
    ...
    mockRedis.xpending.mockImplementation((...args: unknown[]) => {
      if (args.length === 5) return Promise.resolve([]);
    ...
    expect(warnSpy).toHaveBeenCalledWith(
      expect.stringContaining('Consumer lag warning'),
    );
