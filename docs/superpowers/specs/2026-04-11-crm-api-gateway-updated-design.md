# CRM API Gateway — Updated Design Spec

**Date:** 2026-04-11
**Status:** Approved
**Replaces:** `docs/superpowers/specs/2026-03-25-crm-api-gateway-design.md`
...
- Lead Service returns `200` but `channel` is `null`, absent, not in the valid enum, or the response body is malformed/unparseable → all treated identically: gateway returns `422 { "error": "channel_resolution_failed" }` without forwarding.
...
| Lead channel null/invalid/missing/malformed | 422 | `{ "error": "channel_resolution_failed" }` |
...
│   ├── lib/
│   │   ├── api-key-cache.ts     # LRU (500 entries, configurable TTL) using lru-cache npm package
...
All `*_URL`, `*_KEY`, and `*_SECRET` values are required at startup — the service fails fast on missing config. Numeric env vars with defaults in the table above are optional.
...
| Package | Role |
|---|---|
| `@fastify/reply-from` | Reverse proxy / SSE streaming |
| `@fastify/rate-limit` | Tiered rate limiting plugin |
| `@ortho/auth-middleware` | RS256 JWT verification + JWKS caching; `must_change_password` enforcement |
| `@ortho/logger` | Structured JSON logging (Pino, Datadog-compatible) |
| `lru-cache` | In-process LRU cache for API key validation results in `lib/api-key-cache.ts` |

**Intentionally excluded:**

| Package | Reason |
|---|---|
| `@ortho/db` / Knex / Drizzle | No database — the gateway has no schema and no migrations |
| `@ortho/event-bus` | No EventBridge — the gateway publishes no events and subscribes to none |
| BullMQ | No job queue — every request is synchronous proxy + optional VPC side-call |

This service is intentionally thin. Any addition of DB access, event publishing, or background workers is out of scope by design.

---

## 14. Test Coverage

Four integration test files covering all gateway-specific behaviours:

| File | Coverage |
|---|---|
| `auth.test.ts` | JWT valid / invalid / expired; `ak_` valid / invalid / cached (LRU hit and miss); header stripping (verify `X-User-*` cannot be spoofed by client); `must_change_password` blocks all routes |
| `pipeline.test.ts` | Override RBAC for all five roles (all permitted + all blocked); channel resolution happy path; all failure modes (Lead Service `404`, `5xx`, null channel, malformed response) |
| `referrals.test.ts` | Public routes bypass JWT (no `Authorization` needed); JWT enforcement on non-public referral routes; `followRedirects: false` on click redirect route returns `302` as-is |
| `rate-limit.test.ts` | Per-tier limit enforcement (IP for public, `sub` for JWT, `keyHash` for API key); `/health` is exempt from rate limiting |
