...
const LeadsQuerySchema = Type.Intersect([
  SharedQuerySchema,
  Type.Object({
...
export async function leadsRoutes(app: FastifyInstance, options: { pool: Pool }): Promise<void> {
  app.get('/analytics/metrics/leads', {
    ...
    let groupBy: string;
    if (dExpr) {
      selectCols = `${dExpr} AS date, location_id, channel, SUM(count)::int AS count, SUM(archived)::int AS archived`;
      groupBy = `GROUP BY ${dExpr}, location_id, channel`;
    } else {
      selectCols = `location_id, channel, SUM(count)::int AS count, SUM(archived)::int AS archived`;
      groupBy = `GROUP BY location_id, channel`;
    }

    const countParams = [...params];
    const countSql = `SELECT COUNT(*) FROM (SELECT 1 FROM platform_analytics.metrics_leads_daily WHERE ${where} ${groupBy}) sub`;
    const countResult = await options.pool.query(countSql, countParams);
    const total = parseInt(countResult.rows[0].count, 10);

    params.push(pageSize, offset);
    const dataSql = `SELECT ${selectCols} FROM platform_analytics.metrics_leads_daily WHERE ${where} ${groupBy} ORDER BY ${dExpr ? `${dExpr}, ` : ''}location_id, channel LIMIT $${params.length - 1} OFFSET $${params.length}`;
