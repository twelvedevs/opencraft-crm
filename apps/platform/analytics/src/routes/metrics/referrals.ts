...
export async function referralsRoutes(app: FastifyInstance, options: { pool: Pool }): Promise<void> {
  app.get('/analytics/metrics/referrals', {
    ...
    let groupBy: string;
    if (dExpr) {
      selectCols = `${dExpr} AS date, location_id, SUM(count)::int AS count`;
      groupBy = `GROUP BY ${dExpr}, location_id`;
    } else {
      selectCols = `location_id, SUM(count)::int AS count`;
      groupBy = `GROUP BY location_id`;
    }

    const countParams = [...params];
    const countSql = `SELECT COUNT(*) FROM (SELECT 1 FROM platform_analytics.metrics_referrals_daily WHERE ${where} ${groupBy}) sub`;
    const countResult = await options.pool.query(countSql, countParams);
    const total = parseInt(countResult.rows[0].count, 10);

    params.push(pageSize, offset);
    const dataSql = `SELECT ${selectCols} FROM platform_analytics.metrics_referrals_daily WHERE ${where} ${groupBy} ORDER BY ${dExpr ? `${dExpr}, ` : ''}location_id LIMIT $${params.length - 1} OFFSET $${params.length}`;
