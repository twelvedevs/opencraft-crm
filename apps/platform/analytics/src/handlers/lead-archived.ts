...
import { upsertLeadDaily } from '../repositories/rollups.js';

export async function handleLeadArchived(event: OrthoEvent, pool: Pool): Promise<void> {
  ...
  try {
    ...
    if (inserted) {
      await upsertLeadDaily(client, { date, location_id, channel, archived_delta: 1 });
    }
