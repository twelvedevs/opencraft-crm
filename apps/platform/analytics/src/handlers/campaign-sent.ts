...
export async function handleCampaignSent(event: OrthoEvent, pool: Pool): Promise<void> {
  ...
  try {
    ...
    if (inserted) {
      await upsertCampaignDaily(client, { date, campaign_id, location_id, sent_delta: 1 });
    }
