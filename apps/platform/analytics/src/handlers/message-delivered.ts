...
export async function handleMessageDelivered(event: OrthoEvent, pool: Pool): Promise<void> {
  ...
  try {
    ...
    if (inserted) {
      await upsertMessageDaily(client, { date, location_id, delivered_delta: 1 });
    }
