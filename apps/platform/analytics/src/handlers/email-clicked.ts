...
export async function handleEmailClicked(event: OrthoEvent, pool: Pool): Promise<void> {
  ...
  try {
    ...
    if (inserted) {
      await upsertCampaignDaily(client, { date, campaign_id, location_id, clicked_delta: 1 });
    }
