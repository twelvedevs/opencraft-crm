...
const { handleMessageFailed } = await import('../../../src/handlers/message-failed.js');
...
describe('handleMessageFailed', () => {
  it('calls insertEvent with correct params', async () => {
    ...
    await handleMessageFailed(baseEvent, pool);
  ...
  it('calls upsertMessageDaily with failed_delta=1 when inserted=true', async () => {
    mockInsertEvent.mockResolvedValue({ inserted: true });
    mockUpsertMessageDaily.mockResolvedValue(undefined);
    const pool = makePool();
    ...
    expect(params.failed_delta).toBe(1);
  });
