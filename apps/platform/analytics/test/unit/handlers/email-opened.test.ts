...
const { handleEmailOpened } = await import('../../../src/handlers/email-opened.js');
...
describe('handleEmailOpened', () => {
  it('calls insertEvent with entity_type=email', async () => {
    ...
    await handleEmailOpened(baseEvent, pool);
  ...
  it('calls upsertCampaignDaily with opened_delta=1 when inserted=true', async () => {
    mockInsertEvent.mockResolvedValue({ inserted: true });
    mockUpsertCampaignDaily.mockResolvedValue(undefined);
    const pool = makePool();
    ...
    expect(params.opened_delta).toBe(1);
