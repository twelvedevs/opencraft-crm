...
import { applyOptOut, type ContactStatus } from '../../scoring/contact-status.js';

const log = createLogger('crm-lead');

export async function handleOptOutReceived(
  event: OrthoEvent,
  db: Knex,
): Promise<void> {
  ...
  try {
    ...
    log.warn({ phone_number }, 'opt_out.received: invalid phone, skipping');
    return;
  }

  const leads = await leadRepository.findByPhone(db, normalizedPhone);
  if (leads.length === 0) {
    log.warn({ phone: normalizedPhone }, 'opt_out.received: no lead found');
    return;
  }
  ...
  await db.transaction(async (trx) => {
    // 1. Apply opt-out transition
    const newStatus = applyOptOut(lead.contact_status as ContactStatus);
    ...
    // 6. Insert activity
    const occurred_at = payload.opted_out_at
      ? new Date(String(payload.opted_out_at))
      : new Date();
    ...
    await activityRepository.insertActivity(trx, {
      ...
      source_event_id: event.event_id ?? `fallback:opt_out.received:${lead_id}`,
    });
  });
}
