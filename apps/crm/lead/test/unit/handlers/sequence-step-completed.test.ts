...
describe('handleSequenceStepCompleted', () => {
  it('returns early without DB access when entity_type is not lead', async () => {
    const event = makeEvent({ entity_type: 'contact', entity_id: 'contact-1' });

    await handleSequenceStepCompleted(event, db);
  ...
  it('inserts activity when entity_type is lead and lead found', async () => {
    await handleSequenceStepCompleted(makeEvent(), db);
